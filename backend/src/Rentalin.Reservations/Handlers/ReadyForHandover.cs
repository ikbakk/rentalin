...
public sealed class ReadyForHandoverHandler : IRequestHandler<ReadyForHandoverRequest, ReservationResponse>
{
    private readonly IRepository<Reservation> _reservations;
    private readonly IUnitOfWork _unitOfWork;

    public ReadyForHandoverHandler(IRepository<Reservation> reservations, IUnitOfWork unitOfWork)
    {
        _reservations = reservations;
        _unitOfWork = unitOfWork;
    }

    public async Task<ReservationResponse> Handle(ReadyForHandoverRequest request, CancellationToken ct)
    {
