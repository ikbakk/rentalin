...
const steps = [
  ...
  { key: "Active", label: "On Rent", icon: MapPin },
  { key: "Completed", label: "Completed", icon: CheckCircle2 },
]

const statusConfig: Record<string, { bg: string; text: string; label: string }> = {
  Inquiry: { bg: "bg-amber-500/10", text: "text-amber-500", label: "Inquiry" },
...
export function BookingView({ reservationId }: { reservationId: string }) {
    ...
    return (
      <div className="flex min-h-dvh flex-col items-center justify-center gap-4 p-6">
        <div className="flex size-16 items-center justify-center rounded-full bg-destructive/10">
          <Circle className="size-8 text-destructive" />
        </div>
  ...
  const statusInfo = statusConfig[rental.status] || statusConfig.Inquiry
      ...
      <div className="relative mx-auto max-w-lg px-4 pb-8">
        <header className="pt-8 pb-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Booking ID</p>
              <p className="font-mono text-sm text-muted-foreground">{rental.reservationId.slice(0, 8)}</p>
            </div>
        ...
        {showSteps && (
          <Card className="mb-6">
            <CardContent className="p-4">
              <h3 className="mb-4 text-sm font-semibold">Booking Timeline</h3>
